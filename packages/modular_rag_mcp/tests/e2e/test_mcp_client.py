...
def _start_server() -> subprocess.Popen:
    ...
    return subprocess.Popen(
        ...
        # Server logs are intentionally verbose and are not asserted here.
        # Never leave stderr as an unread PIPE: it can fill and block the child.
...
def _send_jsonrpc(
    ...
    # MCP lifecycle requests are ordered: the client must receive the
    # initialize response before it sends notifications/initialized and any
    # tool request. Send each request and wait for its matching response.
    for msg in messages:
        proc.stdin.write(json.dumps(msg) + "\n")
        proc.stdin.flush()
        request_id = msg.get("id")
        if request_id is not None:
            # Native Chroma import happens before stdio workers start. Allow a
            # larger one-time Windows cold-start budget for initialization;
            # subsequent requests retain the test-specific timeout.
            request_timeout = 90.0 if msg.get("method") == "initialize" else timeout
            request_deadline = time.time() + request_timeout
            while (
                _find(responses, request_id) is None
