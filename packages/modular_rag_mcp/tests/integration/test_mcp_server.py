...
def send_and_receive(
    ...
    reader_thread.start()
    # Respect the MCP initialization barrier by waiting for every request's
    ...
    for req in requests:
        ...
        if request_id is not None:
            ...
            while (
                not any(item.get("id") == request_id for item in responses)
...
def test_mcp_server_initialize_stdio() -> None:
    ...
    try:
        lines = send_and_receive(proc, [request], timeout=90.0)
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=5)
...
def test_mcp_server_tools_list_stdio() -> None:
    """Ensure tools/list works after a standards-compliant handshake."""
    ...
    try:
        lines = send_and_receive(proc, requests, timeout=90.0)
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=5)
