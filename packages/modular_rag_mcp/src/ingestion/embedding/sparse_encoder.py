...
class SparseEncoder:
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        # jieba splits identifiers such as ``gpt-4`` and ``deep_learning``.
        # Protect those terms first because they are meaningful model/product
        # identifiers for lexical retrieval.
        protected_pattern = re.compile(
            r"(?i)[a-z0-9]+(?:[-_][a-z0-9]+)+|\d+(?:\.\d+)+"
        )
        protected_tokens = [match.group(0) for match in protected_pattern.finditer(text)]
        text_for_segmentation = protected_pattern.sub(" ", text)

        # Use jieba to segment the text (handles both Chinese and English)
        raw_tokens = jieba.lcut(text_for_segmentation) + protected_tokens
