...
def _optional_string(value: Any) -> Optional[str]:
    """Return a non-empty string and reject dynamic Mock-like objects."""
    if isinstance(value, str) and value.strip():
        cleaned = value.strip()
        upper = cleaned.upper()
        if "YOUR_API_KEY" in upper or "YOUR_ENDPOINT" in upper or "CHANGEME" in upper:
            return None
        return cleaned
    return None
...
class AzureEmbedding(BaseEmbedding):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.deployment_name = _optional_string(
        ...
        # Environment precedence keeps deployments twelve-factor friendly.
        self.api_key = (
            _optional_string(api_key)
            or _optional_string(os.environ.get("AZURE_OPENAI_API_KEY"))
        ...
        self.azure_endpoint = (
            _optional_string(azure_endpoint)
            or _optional_string(os.environ.get("AZURE_OPENAI_ENDPOINT"))
            or _optional_string(getattr(settings.embedding, 'azure_endpoint', None))
        )
        ...
        self.api_version = (
            _optional_string(api_version)
            or _optional_string(getattr(settings.embedding, 'api_version', None))
            or
