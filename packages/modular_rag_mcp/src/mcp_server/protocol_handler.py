...
def _register_default_tools(protocol_handler: ProtocolHandler) -> None:
    ...
    """
    # A caller may provide a partially preconfigured handler. Registration is
    # idempotent so custom handlers can safely override one or more defaults.
...
def create_mcp_server(
    ...
    Args:
        ...
        protocol_handler: Optional pre-configured protocol handler. If supplied,
            its tool registry is preserved as-is. If None, a new one is created.
        register_tools: Whether a newly created handler receives default tools.

    Returns:
        Configured Server instance ready to run.
    """
    owns_handler = protocol_handler is None
    if owns_handler:
    ...
    # A caller-provided handler is an explicit registry boundary: do not mutate
    # it by injecting unrelated defaults.
    if register_tools and owns_handler:
