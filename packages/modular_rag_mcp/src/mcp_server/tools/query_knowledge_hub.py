...
class QueryKnowledgeHubTool:
    ...
    def _load_runtime_dependencies() -> None:
        """Load native/heavy modules on the event-loop thread before workers.

        ChromaDB transitively imports native extensions. Importing those for the
        first time inside ``asyncio.to_thread`` can contend with anyio's stdio
        reader on Windows. Keeping this lazy preserves fast MCP initialization,
        while loading safely only when the query tool is actually used.
        """
    ...
    async def execute(
        ...
        try:
            ...
            # First-use construction performs dynamic imports and creates native
            # Chroma objects. On Windows this must stay on the event-loop thread;
            # doing it in an anyio worker can deadlock with stdio's reader thread.
            # Actual network/database search remains offloaded below.
